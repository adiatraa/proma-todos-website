...
const TodoSection = ({ isAddingTask, setIsAddingTask, sectionName, isOpen, onToggle }) => {
    ...
    return (
        <div>
            <button onClick={onToggle} className="p-5 border-b border-gray-200 flex items-center w-full">
                {isOpen ? <ChevronDownIcon className="mr-2 h-5 w-5 text-gray-900 ml-4" aria-hidden="true" /> : <ChevronRightIcon className="mr-2 h-5 w-5 text-gray-900 ml-4" aria-hidden="true" />}
                <h1 className="ml-4 font-bold text-2xl">{sectionName}</h1>
            </button>
            <div className={`overflow-hidden transition-all duration-300 ease-in-out text-slate-600 ${isOpen ? "block" : "hidden"}`}>
                {todoList.map((todo) => (
                    <div key={todo.id} className="flex flex-col sm:flex-row w-full border-b border-gray-200">
                        ...
                        <div className="p-5 w-full border-l sm:w-60 border-t sm:border-t-0 sm:border-r border-gray-200 overflow-hidden">
                            <input
                                className="w-full p-2 border-transparent focus:outline-none focus:ring-0 focus:border-transparent"
                                name="assignee"
                                value={todo.assignee}
